...
import userReducer, { checkUserAuthThunk, clearAllFormFields, logoutUserThunk, setUser, updateUserThunk, userLoginThunk, userRegisterThunk } from '../userProfileSlice';
...
describe('Тестирование user slice:', () => {
  test('userRegisterThunk.pending', () => {
      const state = userReducer(userInitialState, userRegisterThunk.pending('', registerPayload));
  ...
  test('userRegisterThunk.fulfilled', () => {
    const state = userReducer(userInitialState, userRegisterThunk.fulfilled(mockUser, '', registerPayload));
  ...
  test('userRegisterThunk.rejected', () => {
    const state = userReducer(userInitialState, userRegisterThunk.rejected({ message: 'Регистрация не удалась', name: 'Error' }, '', registerPayload));
    expect(state.loginUserRequest).toBe(false);
    expect(state.isAuthenticated).toBe(false);
  ...
  test('userLoginThunk.pending', () => {
    const state = userReducer(userInitialState, userLoginThunk.pending('', registerPayload));
    expect(state.isAuthenticated).toBe(false);
    expect(state.loginUserRequest).toBe(true);
  });

  test('userLoginThunk.fulfilled', () => {
    const state = userReducer(userInitialState, userLoginThunk.fulfilled(mockUser, '', registerPayload));
    expect(state.loginUserRequest).toBe(false);
    expect(state.isAuthenticated).toBe(true);
    expect(state.user).toEqual(mockUser);
    expect(state.error).toBe('');
  ...
  test('userLoginThunk.rejected', () => {
    ...
    const state = userReducer(userInitialState, userLoginThunk.rejected(error, '', registerPayload));
    expect(state.loginUserRequest).toBe(false);
    expect(state.isAuthenticated).toBe(false);
 ...
 test('logoutUserThunk.pending', () => {
  ...
  test('logoutUserThunk.fulfilled', () => {
    const loggedInState = { ...userInitialState, user: mockUser, isAuthenticated: true };
    const state = userReducer(loggedInState, logoutUserThunk.fulfilled(undefined, '', undefined));
  ...
  test('logoutUserThunk.rejected', () => {
    ...
    const state = userReducer(userInitialState, logoutUserThunk.rejected(error, ''));
    expect(state.loginUserRequest).toBe(false);
    expect(state.isAuthenticated).toBe(false);
  ...
  test('checkUserAuthThunk.fulfilled', () => {
    const state = userReducer(userInitialState, checkUserAuthThunk.fulfilled(mockUser, '', undefined));
    expect(state.loginUserRequest).toBe(false);
    expect(state.isAuthenticated).toBe(true);
    expect(state.user).toEqual(mockUser);
  });
  ...
  test('checkUserAuthThunk.rejected', () => {
    ...
    const state = userReducer(userInitialState, checkUserAuthThunk.rejected(error, ''));
    expect(state.loginUserRequest).toBe(false);
    expect(state.isAuthenticated).toBe(false);
  ...
  test('updateUserThunk.pending', () => {
    const state = userReducer(userInitialState, updateUserThunk.pending('', {}));
    expect(state.loginUserRequest).toBe(true);
  });

  test('updateUserThunk.fulfilled', () => {
    const response = { user: mockUser, success: true };
    const state = userReducer(userInitialState, updateUserThunk.fulfilled(response, '', {}));
    expect(state.loginUserRequest).toBe(false);
    expect(state.user).toEqual(mockUser);
    expect(state.isAuthenticated).toBe(true);
    expect(state.error).toBe('');
  ...
  test('updateUserThunk.rejected', () => {
    ...
    const state = userReducer(userInitialState, updateUserThunk.rejected(error, '', {}));
    expect(state.loginUserRequest).toBe(false);

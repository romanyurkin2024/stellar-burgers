import { mockGetOrderResponse, mockOrder, mockOrderResponse, mockOrders } from '../__mocks__/mockData';
import orderSliceReducer, { clearOrder, orderBurgerThunk, orderDetailsThunk, orderSliceInitialState } from '../orderSlice';
...
describe('Тестирование слайса orderSlice:', () => {
  ...
  test('Тестирование action создания заказа (orderBurgerThunk)', () => {
    const state = orderSliceReducer(undefined, orderBurgerThunk.pending('', []));
  ...
  test('Тестирование успешного action создания заказа (orderBurgerThunk)', () => {
    const state = orderSliceReducer(orderSliceInitialState, orderBurgerThunk.fulfilled(mockOrderResponse, '', []));
  ...
  test('Тестирование неудачного action создания заказа (orderBurgerThunk)', () => {
    const state = orderSliceReducer(orderSliceInitialState, orderBurgerThunk.rejected({name: 'Error', message: 'Ошибка заказа!'}, '', []));
    expect(state.orderRequest).toBe(false);
    expect(state.order).toBeNull();
  ...
  test('Тестирование action получения деталей по заказу (orderDetailsThunk)', () => {
    const state = orderSliceReducer(undefined, orderDetailsThunk.pending('', 0));
    expect(state.orderRequest).toBe(true);
    expect(state.error).toBeNull();
  ...
  test('Тестирование успешного action получения деталей по заказу (orderDetailsThunk)', () => {
    const state = orderSliceReducer(orderSliceInitialState, orderDetailsThunk.fulfilled(mockGetOrderResponse, '', 0));
    expect(state.orderRequest).toBe(false);
  ...
  test('Тестирование неудачного action получения деталей по заказу (orderDetailsThunk)', () => {
    const state = orderSliceReducer(orderSliceInitialState, orderDetailsThunk.rejected({name: 'Error', message: 'Ошибка получения заказов!'}, '', 0));
    expect(state.orderRequest).toBe(false);
    expect(state.feedOrder).toBeNull();
